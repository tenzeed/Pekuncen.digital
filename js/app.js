...
function sanitizeRT(val) {
  let clean = String(val || '').trim();
  return ['29', '30', '31', '32'].includes(clean) ? clean : null;
}
...
// ============================================================
// PENTING: Ganti 2 baris di bawah ini dengan URL & anon key
// project Supabase ANDA SENDIRI (Settings > API di dashboard
// Supabase). JANGAN biarkan nilai placeholder ini terpasang --
...
async function generateFormInputs(rowData) {
  ...
  for (let idx = 0; idx < headersToUse.length; idx++) {
    ...
    if (!rowData && nameLower === 'rt' && session.role === 'Warga' && sanitizeRT(session.rt)) {
      val = sanitizeRT(session.rt);
    }
    ...
    if (nameLower === 'status' && ['Pengaduan','SuratPengantar','Sumbangan'].includes(currentActiveMenu)) {
      ...
      let allowShared = (currentActiveMenu === 'Keuangan' || currentActiveMenu === 'Aset');
      let rtOpts = ['29','30','31','32'].map(r => `<option value="${r}" ${String(val).trim()===r?'selected':''}>RT ${r}</option>`).join('');
      let sharedOpt = allowShared ? `<option value="" ${String(val).trim()===''?'selected':''}>RW 08 (Bersama / Semua RT)</option>` : `<option value="">-- Pilih RT --</option>`;
      inputHtml = `<select class="form-select dynamic-input" data-key="${h}" ${isWargaLocked ? 'disabled' : (allowShared ? '' : 'required')}>
        ${sharedOpt}
        ${rtOpts}
      </select>
      ${isWargaLocked ? `<input type="hidden" class="dynamic-input" data-key="${h}" value="${String(val).trim()}"><small class="text-muted text-[10px] d-block mt-1">*Otomatis mengikuti RT akun Anda.</small>` : `<small class="text-muted text-[10px] d-block mt-1">*Pilih RT pemilik data ini (29/30/31/32)${allowShared ? ', atau biarkan sebagai milik bersama RW' : ''}.</small>`}`;
...
function submitFormBaru(e) {
  ...
  if ('rt' in payload) {
    payload.rt = sanitizeRT(payload.rt);
  }
